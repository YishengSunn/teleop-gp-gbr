...
from launch.actions import DeclareLaunchArgument, IncludeLaunchDescription, TimerAction, Shutdown, OpaqueFunction
...
def generate_launch_description():
    ...
    right_move_to_start_controller = Node(
        package='controller_manager',
        executable='spawner',
        namespace=ns,
        arguments=['right_move_to_start_controller'],
        output='screen',
    )

    left_move_to_start_controller = Node(
        package='controller_manager',
        executable='spawner',
        namespace=ns,
        arguments=['left_move_to_start_controller'],
        output='screen',
    )

    right_joint_impedance_controller = Node(
        package='controller_manager',
        executable='spawner',
        namespace=ns,
        arguments=['right_joint_impedance_controller'],
        output='screen',
        condition=IfCondition(PythonExpression(["'", LaunchConfiguration('control_mode'), "' == 'joint_impedance'"])),

    )

    left_joint_impedance_controller = Node(
        package='controller_manager',
        executable='spawner',
        namespace=ns,
        arguments=['left_joint_impedance_controller'],
        output='screen',
        condition=IfCondition(PythonExpression(["'", LaunchConfiguration('control_mode'), "' == 'joint_impedance'"])),
    )

    right_cartesian_delta_impedance_controller_inactive = Node(
        package='controller_manager',
        executable='spawner',
        namespace=ns,
        arguments=['right_cartesian_delta_impedance_controller', '--inactive'],
        output='screen',
        condition=IfCondition(PythonExpression(["'", LaunchConfiguration('control_mode'), "' == 'joint_impedance'"])),

    )

    left_cartesian_delta_impedance_controller_inactive = Node(
        package='controller_manager',
        executable='spawner',
        namespace=ns,
        arguments=['left_cartesian_delta_impedance_controller', '--inactive'],
        output='screen',
        condition=IfCondition(PythonExpression(["'", LaunchConfiguration('control_mode'), "' == 'joint_impedance'"])),
    )

    right_cartesian_impedance_controller = Node(
        package='controller_manager',
        executable='spawner',
        namespace=ns,
        arguments=['right_cartesian_impedance_controller'],
        output='screen',
        condition=IfCondition(PythonExpression(["'", LaunchConfiguration('control_mode'), "' == 'cartesian_impedance'"])),

    )

    left_cartesian_impedance_controller = Node(
        package='controller_manager',
        executable='spawner',
        namespace=ns,
        arguments=['left_cartesian_impedance_controller'],
        output='screen',
        condition=IfCondition(PythonExpression(["'", LaunchConfiguration('control_mode'), "' == 'cartesian_impedance'"])),
    )

    right_cartesian_delta_impedance_controller = Node(
        package='controller_manager',
        executable='spawner',
        namespace=ns,
        arguments=['right_cartesian_delta_impedance_controller'],
        output='screen',
        condition=IfCondition(PythonExpression(["'", LaunchConfiguration('control_mode'), "' == 'cartesian_delta_impedance'"])),

    )

    left_cartesian_delta_impedance_controller = Node(
        package='controller_manager',
        executable='spawner',
        namespace=ns,
        arguments=['left_cartesian_delta_impedance_controller'],
        output='screen',
        condition=IfCondition(PythonExpression(["'", LaunchConfiguration('control_mode'), "' == 'cartesian_delta_impedance'"])),
    )
    # RViz
    ...
    # validate control mode argument
    def validate_control_mode(context, *args, **kwargs):
        allowed = {"cartesian_impedance", "joint_impedance", "cartesian_delta_impedance"}
        mode = LaunchConfiguration("control_mode").perform(context)

        if mode not in allowed:
            raise RuntimeError(
                f'Invalid control_mode="{mode}". Allowed: {sorted(allowed)}'
            )
        return []   
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            "control_mode",
            description='Choose one: cartesian_impedance | joint_impedance | cartesian_delta_impedance',
        ),


        OpaqueFunction(function=validate_control_mode),
        ...
        TimerAction(
            ...
            actions=[
                ...
                right_cartesian_delta_impedance_controller,
                left_cartesian_delta_impedance_controller,
                right_cartesian_delta_impedance_controller_inactive,
                left_cartesian_delta_impedance_controller_inactive,
            ]
        ),
